...
-- Find Most Common Factors Among High-Stress Individuals
SELECT a.Occupation, 
       AVG(a.Sleep_Hours) AS Avg_Sleep,
       AVG(a.Physical_Activity) AS Avg_Exercise,
       AVG(a.Caffeine_Intake) AS Avg_Caffeine,
       COUNT(a.ID) AS High_Stress_Cases
FROM anxiety_attack_dataset a
JOIN (SELECT ID FROM anxiety_attack_dataset WHERE Stress_Level >= 7) high_stress
ON a.ID = high_stress.ID
GROUP BY a.Occupation
ORDER BY High_Stress_Cases DESC;

-- Check If Therapy Sessions Reduce Stress
SELECT Therapy_Sessions, 
       COUNT(ID) AS Count_People,
       AVG(Stress_Level) AS Avg_Stress_Level,
       CASE 
           WHEN AVG(Stress_Level) < (SELECT AVG(Stress_Level) FROM anxiety_attack_dataset) 
           THEN 'Effective'
           ELSE 'Not Effective'
       END AS Effectiveness
FROM anxiety_attack_dataset
GROUP BY Therapy_Sessions
ORDER BY Therapy_Sessions DESC;
